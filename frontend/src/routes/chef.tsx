...
import { takePendingArtifact, sendToCase } from "@/lib/handoff";
...
function ChefPage() {
  ...
  const copy = () => { copyText(output); setCopied(true); setTimeout(() => setCopied(false), 1200); };
  const copyMarkdownSummary = () => {
    ...
    copyText(md); setNotice("Markdown summary copied"); setTimeout(() => setNotice(""), 1500);
  };
  ...
  return (
    ...
    >
      ...
      <SendToRow targets={[
        { label: "Case Notebook", to: "/case", icon: FileText, onClick: () => sendToCase({ body: output, source: "/chef", kind: "evidence" }) },
      ]} />
    </PageShell>
  );
}
