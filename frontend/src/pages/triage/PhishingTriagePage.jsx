...
function MetricCard({ label, value, tone = "neutral", helper }) {
  ...
  return (
    <article className="p-metric" {...toneAttr}>
      ...
      {helper ? <div style={{ fontSize: "0.65rem", color: "var(--neo-subtle)", marginTop: "0.15rem" }}>{helper}</div> : null}
    </article>
  )
}
        ...
        <article className="p-card p-intake">
          ...
          <div className="p-intake-ft">
            ...
            <div className="p-intake-ft-right">
              <span style={{ fontSize: "0.65rem", fontFamily: "JetBrains Mono, monospace", color: "var(--neo-subtle)", display: "flex", alignItems: "center", gap: "0.25rem" }}>
        ...
        <aside style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
          <article className="p-side-card">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              <div>
                <label style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "0.6rem", fontWeight: 700, textTransform: "uppercase", color: "var(--neo-subtle)", display: "block", marginBottom: "0.35rem" }}>Analysis Mode</label>
                <select
                  value={mode}
  ...
  return (
    <div className="p-result-hd">
      <div className="p-result-hd-left">
        ...
        <p style={{ fontSize: "0.78rem", color: "var(--neo-muted)", display: "flex", alignItems: "center", gap: "0.35rem" }}>
...
function AuthResults({ result }) {
  ...
  return (
    <article className="p-card">
      ...
      <div style={{ padding: "0.5rem 0.75rem 0.75rem", fontFamily: "JetBrains Mono, monospace", fontSize: "0.82rem", color: "var(--neo-subtle)", borderTop: "2px solid var(--p-line)" }}>
      ...
      <div className="p-evidence-strip">
        {rows.slice(0, 10).map((item, index) => (
          <div key={`${item.signal || item.name}-${index}`} className="p-evidence-card" data-rose={severityTone(item.severity) === "bad" ? "" : undefined} data-gold={severityTone(item.severity) === "warn" ? "" : undefined} data-emerald={severityTone(item.severity) === "good" ? "" : undefined}>
            ...
            <p style={{ fontSize: "0.7rem", color: "var(--neo-muted)", margin: "0.15rem 0 0" }}>{item.evidence || item.meaning || item.why}</p>
      ...
      <div style={{ padding: "0.75rem" }}>
        <div style={{ marginBottom: "0.75rem" }}>
          ...
          <div className="p-ioc-strip">
            ...
            )) : <span style={{ fontSize: "0.7rem", color: "var(--neo-subtle)" }}>No URL inventory extracted.</span>}
          </div>
        </div>
        <div>
  ...
  return (
    <article className="p-card">
      ...
      <div className="p-chain-grid">
        ...
        )) : <div style={{ padding: "0.75rem", fontSize: "0.7rem", color: "var(--neo-subtle)" }}>No evidence chain generated.</div>}
      </div>
    </article>
  )
}
      ...
      <div style={{ padding: "0.75rem" }}>
        {rows.map((row) => (
          <div key={row.field} className="p-auth-row" data-emerald={row.aligned ? "" : undefined} data-rose={!row.aligned ? "" : undefined}>
            ...
            <span>{row.value}</span>
            <small style={{ gridColumn: "1 / -1", fontSize: "0.78rem", color: "var(--neo-subtle)" }}>{row.domain} · {row.meaning}</small>
          </div>
        ))}
      </div>
    </article>
    ...
    <article className="p-card">
      ...
      <div className="p-engine-grid">
        <div className="p-engine-cell">
          ...
          <small style={{ fontSize: "0.78rem", color: "var(--neo-subtle)" }}>From: {result.headers.from || "N/A"} · Mode: {MODE_LABELS[mode]}</small>
        </div>
        <div className="p-engine-cell">
          ...
          <small style={{ fontSize: "0.78rem", color: "var(--neo-subtle)" }}>SPF · DKIM · DMARC</small>
        </div>
        <div className="p-engine-cell">
          ...
          <small style={{ fontSize: "0.78rem", color: "var(--neo-subtle)" }}>{result.iocs.emails.length} emails · {result.iocs.hashes.length} hashes</small>
        </div>
        <div className="p-engine-cell">
          ...
          <small style={{ fontSize: "0.78rem", color: "var(--neo-subtle)" }}>{sourceCount} evidence source(s) · {urlRollup.summary || result.decision.reliability}</small>
        </div>
      </div>
...
function ScoreBreakdownPanel({ result }) {
  ...
  return (
    <article className="p-card">
      ...
      <p style={{ padding: "0 0.75rem", fontSize: "0.85rem", color: "var(--neo-muted)", margin: "0.5rem 0" }}>{score.explanation}</p>
      <div style={{ padding: "0 0.75rem 0.75rem" }}>
        {rows.length ? rows.map((row, index) => (
          <div key={`${row.signal}-${index}`} className="p-score-row" data-rose={severityTone(row.severity) === "bad" ? "" : undefined} data-gold={severityTone(row.severity) === "warn" ? "" : undefined}>
            ...
            {row.analyst_note ? <span className="p-score-note" style={{ color: "var(--neo-subtle)" }}>{row.analyst_note}</span> : null}
          </div>
        )) : <span style={{ fontSize: "0.7rem", color: "var(--neo-subtle)" }}>No weighted score drivers returned.</span>}
      </div>
      {dampeners.length ? (
        ...
        {groups.map(([label, items]) => (
          <div key={label} className="p-ioc-col">
            ...
            <div className="p-ioc-col-body">
              ...
              }) : <em style={{ fontSize: "0.82rem", color: "var(--neo-subtle)" }}>None extracted</em>}
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <article className="p-card">
      ...
      <div className="p-evidence-strip">
        {visibleRows.length ? visibleRows.map((finding, index) => (
          <div key={`${finding.name}-${index}`} className="p-evidence-card" data-rose={severityTone(finding.severity) === "bad" ? "" : undefined} data-gold={severityTone(finding.severity) === "warn" ? "" : undefined}>
            <div className="p-evidence-card-hd">
              ...
              {finding.source ? <small style={{ fontSize: "0.72rem", color: "var(--neo-subtle)" }}>{finding.source}</small> : null}
            </div>
            <strong>{finding.name}</strong>
            <p style={{ fontSize: "0.7rem", color: "var(--neo-muted)", margin: "0.15rem 0 0" }}>{finding.evidence}</p>
        ...
        )) : <span style={{ padding: "0.75rem", display: "block", fontSize: "0.7rem", color: "var(--neo-subtle)" }}>No weighted findings were generated.</span>}
      </div>
      {limit && rows.length > limit ? <div style={{ padding: "0.5rem 0.75rem 0.75rem", fontSize: "0.78rem", color: "var(--neo-subtle)" }}>+{rows.length - limit} more findings available in advanced details.</div> : null}
    </article>
  )
}
...
function FieldRow({ label, value, note, tone, mono }) {
  ...
  return (
    <div className="p-field-row">
      ...
      {note ? <span style={{ fontSize: "0.78rem", color: "var(--neo-subtle)", gridColumn: "1 / -1", marginTop: "-0.2rem" }}>{note}</span> : null}
    </div>
  )
}
  ...
  return (
    <article className="p-card">
      ...
      <div className="p-field-grid">
        ...
        )) : <span style={{ padding: "0.75rem", fontSize: "0.82rem", color: "var(--neo-subtle)" }}>No strong lure keyword extracted.</span>}
...
function SocTriageResults({ result, onSendArtifact, onCopy, setPage, rawInput, mode }) {
  return (
    <section className="p-results">
      <article className="p-verdict-card">
      ...
      <article className="p-card">
        <div className="p-card-hd"><ShieldAlert />Triage actions</div>
        <div className="p-actions-bar">
