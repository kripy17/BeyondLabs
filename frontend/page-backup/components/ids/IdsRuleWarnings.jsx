      ...
      {warnings.length ? (
        <div className="space-y-2">
          {warnings.map((warning) => (
            <p key={warning} className="ba-card" style={{ borderColor: "color-mix(in srgb, #f59e0b 30%, transparent)", background: "color-mix(in srgb, #f59e0b 10%, transparent)", color: "#f59e0b" }}>
              <AlertTriangle className="mr-2 inline h-4 w-4" />{warning}
            </p>
          ))}
        </div>
      ) : (
        <p className="ba-card" style={{ borderColor: "color-mix(in srgb, #10b981 30%, transparent)", background: "color-mix(in srgb, #10b981 10%, transparent)", color: "#10b981" }}>
